...
public sealed class UnixDomainMasteredConnexon : IConnexon {
    public static string DefaultSocketDirectory {
        get => Path.Combine(Path.GetTempPath(),"glycoprotein_mastered");
    }
    ...
    public CancellationToken CancellationToken {
        get => (_cts ?? throw new InvalidOperationException("UnixDomainMasteredConnexon is not started.")).Token;
    }
    ...
    public void Stop() {
        ...
        try { } finally { _reconnectLock.Release(); }
        ...
        if (!_isHub) return;
        try { File.Delete(_hubSocketPath); }
        catch {
            // ignored
        }
    }
    ...
    async Task HubAcceptLoopAsync(CancellationToken ct) {
        try {
            while (!ct.IsCancellationRequested) {
                ...
                lock (_clientsLock) { _clients.Add(stream); }
    ...
    async Task HubReadFromClientAsync(NetworkStream stream, CancellationToken ct) {
        try {
            ...
            lock (_clientsLock) { _clients.Remove(stream); }
            stream.Dispose();
        }
    }
    ...
    async Task HubBroadcastAsync(byte[] data, CancellationToken ct) {
        ...
        foreach (NetworkStream client in clients) {
            ...
            catch { /* cleanup in HubReadFromClientAsync */ }
        }
    }
