...
public sealed class MulticastConnexon(IPEndPoint endPoint) : IConnexon {
    ...
    public CancellationToken CancellationToken { get => (_cts ?? throw new InvalidOperationException("MulticastConnexon is not started.")).Token; }
    ...
    public async Task SendBytesAsync(byte[] data,CancellationToken ct = default) {
        ...
        switch (data.Length) {
            case > MaxPayloadSize:
                throw new
                    InvalidOperationException($"Message size {data.Length} exceeds maximum UDP payload size {MaxPayloadSize:N0} bytes. Reduce the number of fields or split into multiple smaller services.");
